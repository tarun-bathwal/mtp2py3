...
class Mapping:
	...
	def create_mappings(self):
		...
		for (key,value) in list(ItemsToMaybeKeep.items()):
			if value >= self.minReview:
				self.ItemIDMap[key] 	= count
				self.RItemIDMap[count] 	= key
				count 			= count + 1

		temp_vocab = sorted(list(self.Vocublary.items()), key = operator.itemgetter(1))
		...
		for (key,value) in list(self.Vocublary.items()):

			self.WordIDMap[key] 	= count
			self.RWordIDMap[count] 	= key
			count 			= count + 1

		X = sorted(list(self.Vocublary.items()), key = operator.itemgetter(1))
